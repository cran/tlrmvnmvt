pmvn.tlr <- function(lower=-Inf, upper=Inf, mean=0, sigma=NULL, 
                    geom=NULL, type="matern", para=NULL, uselog2=FALSE,
                    m=64, epsl=1e-3, N=499)
{
  ...
  if(mtd == 1)
  {
    tlrmvn_internal(lower, upper, sigma, uselog2, m, epsl, N)
  }else
  {
    tlrmvn_internal2(lower, upper, geom, kernelType, para[1:3], para[4], uselog2, m, epsl, N)
  }
}